...
function getCount(aData){
  var fnd=aData.match(/"n":(\d+)\S+?"INBOX"/);
  if(fnd){
    return fnd[1];
  }else{
    return -1;
...
function process(aHttpChannel, aData) {
  ...
  case ST_PRE_RES:
    ...
    var fnd=aData.match(/name=snonce value="(\S+?)"[\s\S]+?name=stime value="(\S+?)"/);
    if(!fnd)return true;
    ...
    this.getHtml(this.loginData[LOGIN_URL],this.loginData[LOGIN_POST]
        +"&app=bt&return_url=https%3A%2F%2Fwebmail.excite.com%2Fcgi-bin%2Flogin_sso.cgi&snonce="+encodeURIComponent(fnd[1])
        +"&stime="+encodeURIComponent(fnd[2])+"&timeskew="+Skew+"&crep="+encodeURIComponent(Res)
        +"&jerror=none&password="+dummyPassword);
    return false;
  case ST_LOGIN_RES:
    this.getHtml("https://webmail.excite.com/cgi-bin/login_sso.cgi");
    return false;
  case ST_LOGIN_RES+1:
